...
$updateSQL = sprintf("UPDATE EXAMHOLD2 SET TEMP = '%s', PULSE = '%s', RESPRATE = '%s', RESPCHAR = '%s', MUCOUSM = '%s', CRT = '%s', CRTNORMAL='%s', PULSENORM = '%s', RESPNORM = '%s', ATTITUDE = '%s', HYDRATION = '%s', HYDRPC = '%s', TPRMEMO = '%s', BLOODPRES = '%s' WHERE PETNO = '$patient'", 
				...
				$_POST['hydration'], 
				$_POST['hydrpc'], 
				mysqli_real_escape_string($mysqli_link, $_POST['tprmemo']), 
				$_POST['bloodpres'], 
				$_GET['client'], 
				$_GET['patient']);
...
<table width="500" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr>
    ...
    <input name="temp" type="text" class="Inputright" id="temp" value="<?php if (!empty($row_EXAM['TEMP'])){echo $row_EXAM['TEMP'];} else {echo '38.5' ;} ?>" size="6" maxlength="6" onfocus="InputOnFocus(this.id)" onblur="InputOnBlur(this.id)"/>    </td>
  ...
  <tr>
    ...
    <td colspan="2" align="left" class="Labels">
    <input name="pulse" type="text" class="Inputright" id="pulse" value="<?php if (!empty($row_EXAM['PULSE'])) {echo $row_EXAM['PULSE']; } else {echo '130' ;}  ?>" size="6" maxlength="6" onfocus="InputOnFocus(this.id)" onblur="InputOnBlur(this.id)"/>    </td>
  ...
  <tr>
    ...
    <td colspan="2" align="left" class="Labels">
    <input name="resprate" type="text" class="Inputright" id="resprate" value="<?php if (!empty($row_EXAM['RESPRATE'])) {echo $row_EXAM['RESPRATE'];} else {echo '  ' ;} ?>" size="6" maxlength="6" onfocus="InputOnFocus(this.id)" onblur="InputOnBlur(this.id)"/>    </td>
    <td colspan="5" class="Verdana12">
    <label>
  ...
  <tr>
    ...
    <td colspan="7" align="left" class="Labels"><input name="mucousm" type="text" class="Input" id="mucousm" value="<?php if (!empty($row_EXAM['MUCOUSM'])) {echo $row_EXAM['MUCOUSM'];} else {echo 'Pink'; } ?>" size="15" maxlength="15" onfocus="InputOnFocus(this.id)" onblur="InputOnBlur(this.id)"/></td>
    </tr>
  <tr>
    <td height="25" class="Labels">&nbsp;</td>
    ...
    <td colspan="5" class="Verdana12">
    <label>
    <input type="checkbox" name="crtnormal" id="crtnormal" onclick="crt()" value="1" <?php if ( $row_EXAM['CRTNORMAL']=='1' || empty($row_EXAM['CRTNORMAL'])){echo "CHECKED";}; ?>/>
    &lt;1</label>    </td>
    </tr>
  <tr>
    <td height="25" class="Labels">&nbsp;</td>
